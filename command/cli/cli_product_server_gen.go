...
func init() {
	listParam := params.NewListProductServerParam()
	readParam := params.NewReadProductServerParam()
		...
		Subcommands: []*cli.Command{
			{
					...
					if strings.HasPrefix(prev, "-") {
						...
						if strings.Contains(prev, "=") {
							if strings.HasPrefix(cur, "-") {
								...
								completion.ProductServerListCompleteArgs(ctx, listParam, cur, prev, commandName)
								return
							}
						}
						...
						for _, flag := range c.App.Command(commandName).Flags {
							...
							if exists {
								if _, ok := flag.(*cli.BoolFlag); ok {
									...
									completion.ProductServerListCompleteFlags(ctx, listParam, name, cur)
									return
								}
							}
						}
				...
				Action: func(c *cli.Context) error {
					...
					return funcs.ProductServerList(ctx, listParam)
			...
			},
			{
					...
					if strings.HasPrefix(prev, "-") {
						...
						if strings.Contains(prev, "=") {
							if strings.HasPrefix(cur, "-") {
								...
								completion.ProductServerReadCompleteArgs(ctx, readParam, cur, prev, commandName)
								return
							}
						}
						...
						for _, flag := range c.App.Command(commandName).Flags {
							...
							if exists {
								if _, ok := flag.(*cli.BoolFlag); ok {
									...
									// prev is flag , call completion func of each flags
									completion.ProductServerReadCompleteFlags(ctx, readParam, name, cur)
									return
								}
							}
						}
				...
				Action: func(c *cli.Context) error {
					...
					// Run command with params
					return funcs.ProductServerRead(ctx, readParam)
