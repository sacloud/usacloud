					...
					if strings.HasPrefix(prev, "-") {
						...
						if strings.Contains(prev, "=") {
							if strings.HasPrefix(cur, "-") {
								...
								completion.ZoneListCompleteArgs(ctx, listParam, cur, prev, commandName)
								return
							}
						}
						...
						for _, flag := range c.App.Command(commandName).Flags {
							...
							if exists {
								if _, ok := flag.(*cli.BoolFlag); ok {
									...
									completion.ZoneListCompleteFlags(ctx, listParam, name, cur)
									return
								}
							}
						}
					...
					if strings.HasPrefix(prev, "-") {
						...
						if strings.Contains(prev, "=") {
							if strings.HasPrefix(cur, "-") {
								...
								completion.ZoneReadCompleteArgs(ctx, readParam, cur, prev, commandName)
								return
							}
						}
						...
						for _, flag := range c.App.Command(commandName).Flags {
							...
							if exists {
								if _, ok := flag.(*cli.BoolFlag); ok {
									...
									// prev is flag , call completion func of each flags
									completion.ZoneReadCompleteFlags(ctx, readParam, name, cur)
									return
								}
							}
						}
