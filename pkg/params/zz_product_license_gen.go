...
func (p *ListProductLicenseParam) Initialize(in Input, args []string, config *config.Config) error {
	p.input = in
	p.config = config
...
func (p *ReadProductLicenseParam) Initialize(in Input, args []string, config *config.Config) error {
	p.input = in
	p.config = config
