...
func (p *ListProductInternetParam) Initialize(in Input, args []string, config *config.Config) error {
	p.input = in
	p.config = config
...
func (p *ReadProductInternetParam) Initialize(in Input, args []string, config *config.Config) error {
	p.input = in
	p.config = config
