...
func (p *ListProductServerParam) Initialize(in Input, args []string, config *config.Config) error {
	p.input = in
	p.config = config
...
func (p *ReadProductServerParam) Initialize(in Input, args []string, config *config.Config) error {
	p.input = in
	p.config = config
