...
func regionListCmd() *cobra.Command {
	...
	fs.IntVarP(&regionListParam.From, "from", "", 0, "set offset (aliases: offset)")
	fs.IntVarP(&regionListParam.Max, "max", "", 0, "set limit (aliases: limit)")
	...
	fs.StringVarP(&regionListParam.OutputType, "output-type", "o", "", "Output type [table/json/csv/tsv] (aliases: out)")
	fs.StringSliceVarP(&regionListParam.Column, "column", "", []string{}, "Output columns(using when '--output-type' is in [csv/tsv] only) (aliases: col)")
	fs.BoolVarP(&regionListParam.Quiet, "quiet", "q", false, "Only display IDs")
	fs.StringVarP(&regionListParam.Format, "format", "", "", "Output format(see text/template package document for detail) (aliases: fmt)")
...
func regionReadCmd() *cobra.Command {
	...
	fs.StringVarP(&regionReadParam.OutputType, "output-type", "o", "", "Output type [table/json/csv/tsv] (aliases: out)")
	fs.StringSliceVarP(&regionReadParam.Column, "column", "", []string{}, "Output columns(using when '--output-type' is in [csv/tsv] only) (aliases: col)")
	fs.BoolVarP(&regionReadParam.Quiet, "quiet", "q", false, "Only display IDs")
	fs.StringVarP(&regionReadParam.Format, "format", "", "", "Output format(see text/template package document for detail) (aliases: fmt)")
