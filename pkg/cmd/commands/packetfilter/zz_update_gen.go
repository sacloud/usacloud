...
func (p *updateParameter) CleanupEmptyValue(fs *pflag.FlagSet) {
	...
	if !fs.Changed("expressions") {
		p.ExpressionsData = nil
	}
}
...
func (p *updateParameter) buildFlags(fs *pflag.FlagSet) {
	...
	if p.ExpressionsData == nil {
		p.ExpressionsData = pointer.NewString("")
	}
	...
	fs.StringVarP(p.ExpressionsData, "expressions", "", "", "(aliases: --rules)")
...
func (p *updateParameter) buildFlagsUsage(cmd *cobra.Command) {
	...
	}
	{
		...
		fs.SortFlags = false
		fs.AddFlag(cmd.LocalFlags().Lookup("expressions"))
		sets = append(sets, &core.FlagSet{
			Title: "Packet-Filter-specific options",
			Flags: fs,
		})
	}
	{
