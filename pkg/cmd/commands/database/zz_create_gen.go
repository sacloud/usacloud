...
func (p *createParameter) buildFlags(fs *pflag.FlagSet) {
	...
	fs.StringVarP(&p.ReplicaUserPassword, "replica-user-password", "", p.ReplicaUserPassword, "(*required) ")
	fs.BoolVarP(&p.EnableWebUI, "enable-web-ui", "", p.EnableWebUI, "")
	fs.BoolVarP(&p.EnableBackup, "enable-backup", "", p.EnableBackup, "")
	fs.StringSliceVarP(&p.BackupWeekdays, "backup-weekdays", "", p.BackupWeekdays, "(*required) options: [all/sun/mon/tue/wed/thu/fri/sat]")
