...
func (p *createParameter) buildFlags(fs *pflag.FlagSet) {
	...
	fs.VarP(core.NewIDFlag(&p.PlanID, &p.PlanID), "plan-id", "", "(*required: --class or --plan-id)")
	fs.StringVarP(&p.Class, "class", "", p.Class, "(*required: --class or --plan-id) options: [dynamic/ms_windows]")
...
func (p *createParameter) buildFlagsUsage(cmd *cobra.Command) {
	...
	}
	{
		...
		fs.AddFlag(cmd.LocalFlags().Lookup("plan-id"))
		sets = append(sets, &core.FlagSet{
			Title: "Private-Host-specific options",
			Flags: fs,
		})
	}
	{
...
func (p *createParameter) setCompletionFunc(cmd *cobra.Command) {
	cmd.RegisterFlagCompletionFunc("class", util.FlagCompletionFunc("dynamic", "ms_windows"))
