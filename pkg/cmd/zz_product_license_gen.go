...
func productLicenseListCmd() *cobra.Command {
	...
	fs.IntVarP(&productLicenseListParam.From, "from", "", 0, "set offset (aliases: offset)")
	fs.IntVarP(&productLicenseListParam.Max, "max", "", 0, "set limit (aliases: limit)")
	...
	fs.StringVarP(&productLicenseListParam.OutputType, "output-type", "o", "", "Output type [table/json/csv/tsv] (aliases: out)")
	fs.StringSliceVarP(&productLicenseListParam.Column, "column", "", []string{}, "Output columns(using when '--output-type' is in [csv/tsv] only) (aliases: col)")
	fs.BoolVarP(&productLicenseListParam.Quiet, "quiet", "q", false, "Only display IDs")
	fs.StringVarP(&productLicenseListParam.Format, "format", "", "", "Output format(see text/template package document for detail) (aliases: fmt)")
	...
	fs.SetNormalizeFunc(productLicenseListNormalizeFlagNames)
...
func productLicenseReadCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !productLicenseReadParam.Assumeyes {
				...
				result, err := util.ConfirmContinue("read", ctx.IO().In(), ctx.IO().Out())
				if err != nil || !result {
					return err
				}
			}
	...
	fs.StringVarP(&productLicenseReadParam.OutputType, "output-type", "o", "", "Output type [table/json/csv/tsv] (aliases: out)")
	fs.StringSliceVarP(&productLicenseReadParam.Column, "column", "", []string{}, "Output columns(using when '--output-type' is in [csv/tsv] only) (aliases: col)")
	fs.BoolVarP(&productLicenseReadParam.Quiet, "quiet", "q", false, "Only display IDs")
	fs.StringVarP(&productLicenseReadParam.Format, "format", "", "", "Output format(see text/template package document for detail) (aliases: fmt)")
	...
	fs.SetNormalizeFunc(productLicenseReadNormalizeFlagNames)
