...
func NFSShutdown(ctx cli.Context, params *params.ShutdownNFSParam) error {

	client := ctx.GetAPIClient()
	api := client.GetNFSAPI()
	p, e := api.Read(params.Id)
	if e != nil {
		return fmt.Errorf("NFSShutdown is failed: %s", e)
	}
	...
	if err != nil {
		return fmt.Errorf("NFSShutdown is failed: %s", err)
	}
