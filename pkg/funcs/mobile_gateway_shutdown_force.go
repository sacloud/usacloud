...
func MobileGatewayShutdownForce(ctx cli.Context, params *params.ShutdownForceMobileGatewayParam) error {

	client := ctx.GetAPIClient()
	api := client.GetMobileGatewayAPI()
	p, e := api.Read(params.Id)
	if e != nil {
		return fmt.Errorf("MobileGatewayShutdownForce is failed: %s", e)
	}
	...
	if err != nil {
		return fmt.Errorf("MobileGatewayShutdownForce is failed: %s", err)
	}
