...
func Update(ctx cli.Context, params *params.UpdateAutoBackupParam) error {
	...
	p := &sacloud.AutoBackupUpdateRequest{
		BackupSpanWeekdays:      data.BackupSpanWeekdays,
		MaximumNumberOfArchives: data.MaximumNumberOfArchives,
		Name:                    data.Name,
		Description:             data.Description,
		Tags:                    data.Tags,
		IconID:                  data.IconID,
	}
	if params.Changed("Weekdays") {
		p.BackupSpanWeekdays = weekday.FromStrings(params.Weekdays)
	}
	if params.Changed("Generation") {
		p.MaximumNumberOfArchives = params.Generation
	}
