...
func Shutdown(ctx cli.Context, params *params.ShutdownDatabaseParam) error {
	...
	err = ctx.ExecWithProgress(func() error {
		if err := client.Shutdown(ctx, ctx.Zone(), params.Id, &sacloud.ShutdownOption{Force: false}); err != nil {
			return err
		}
