...
func Delete(ctx cli.Context, params *params.DeleteObjectStorageParam) error {
	if ctx.NArgs() != 1 {
	...
	if key == nil {
		...
		return deleteRecursive(ctx, path, bucket)
	}
	// path is file
	err = del(ctx, path, bucket)
	if err != nil {
		return fmt.Errorf("ObjectStorageDelete is failed: %s", err)
	}
...
func deleteRecursive(ctx cli.Context, path string, bucket *s3.Bucket) error {
	...
	for _, content := range res.Contents {
		err := del(ctx, content.Key, bucket)
		if err != nil {
			return err
		}
	}
	...
	for _, pref := range res.CommonPrefixes {
		err := deleteRecursive(ctx, pref, bucket)
		if err != nil {
			return nil
		}
	}
...
func del(ctx cli.Context, path string, bucket *s3.Bucket) error {
	err := bucket.Del(path)
