...
func Put(ctx cli.Context, params *params.PutObjectStorageParam) error {
	useStdIn := false
	srcPath := ""
	destPath := ""
	...
	if useStdIn {
		putFunc = func() error {
			return putReaderMultiNonSeeker(destPath, ctx.IO().In(), bucket, params.ContentType)
		}
	} else {
		if srcInfo.IsDir() {
			if !params.Recursive {
			...
			putFunc = func() error {
				return putRecursive(destPath, srcPath, srcPath, params.Recursive, bucket, params.ContentType)
			}

		} else {
			putFunc = func() error {
				return put(destPath, srcPath, bucket, params.ContentType)
			}
		}
	}
...
func putRecursive(remotePath, baseDir, targetDir string, rec bool, bucket *s3.Bucket, contentType string) error {
	...
	for _, fi := range entries {
		...
		if fi.IsDir() {
			err := putRecursive(dest, baseDir, src, rec, bucket, contentType)
			if err != nil {
				return err
			}
		} else {
			err := put(dest, src, bucket, contentType)
			if err != nil {
				return err
			}
		}
...
func put(destPath, srcPath string, bucket *s3.Bucket, contentType string) error {
	...
	return putReader(destPath, file, bucket, contentType)
}

func putReader(destPath string, file *os.File, bucket *s3.Bucket, contentType string) error {
	fi, err := file.Stat()
	if err != nil {
		return err
	}
	if fi.Size() > 8*1024*1024 { // over 8MB
		return putReaderMulti(destPath, file, bucket, contentType)
	}
...
func putReaderMulti(destPath string, file *os.File, bucket *s3.Bucket, contentType string) error {
...
func putReaderMultiNonSeeker(destPath string, file *os.File, bucket *s3.Bucket, contentType string) error {
