...
func DNSList(ctx cli.Context, params *params.ListDNSParam) error {
	...
	for i := range res.CommonServiceDNSItems {
		...
		if !params.GetCommandDef().Params["tags"].FilterFunc(list, &res.CommonServiceDNSItems[i], params.Tags) {
			continue
		}

		list = append(list, &res.CommonServiceDNSItems[i])
	}
