...
func List(ctx cli.Context, params *params.ListCouponParam) error {
	couponClient := sacloud.NewCouponOp(ctx.Client())
	authClient := sacloud.NewAuthStatusOp(ctx.Client())

	account, err := authClient.Read(ctx)
	if err != nil {
		return err
	}

	res, err := couponClient.Find(ctx, account.AccountID)
	if err != nil {
		return err
	}

	var list []interface{}
	for _, coupon := range res.Coupons {
