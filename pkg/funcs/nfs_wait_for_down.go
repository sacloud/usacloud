...
func NFSWaitForDown(ctx cli.Context, params *params.WaitForDownNFSParam) error {

	client := ctx.GetAPIClient()
	api := client.GetNFSAPI()
	p, e := api.Read(params.Id)
	if e != nil {
		return fmt.Errorf("NFSWaitForDown is failed: %s", e)
	}
	...
	if err != nil {
		return fmt.Errorf("NFSWaitForDown is failed: %s", err)
	}
