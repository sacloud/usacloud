...
func ObjectStorageDelete(ctx cli.Context, params *params.DeleteObjectStorageParam) error {
	if ctx.NArgs() != 1 {
...
func objectStorageDelRecursive(ctx cli.Context, path string, bucket *s3.Bucket) error {
...
func objectStorageDel(ctx cli.Context, path string, bucket *s3.Bucket) error {
	err := bucket.Del(path)
