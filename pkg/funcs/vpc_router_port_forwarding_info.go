...
func VPCRouterPortForwardingInfo(ctx cli.Context, params *params.PortForwardingInfoVPCRouterParam) error {
	...
	if !p.HasPortForwarding() {
		fmt.Fprintf(ctx.IO().Err(), "VPCRouter[%d] don't have any port-forwarding settings\n", params.Id)
		return nil
	}
