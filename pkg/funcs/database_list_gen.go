...
func DatabaseList(ctx cli.Context, params *params.ListDatabaseParam) error {
	...
	res, err := finder.Find()
	if err != nil {
		return fmt.Errorf("DatabaseList is failed: %s", err)
	}

	list := []interface{}{}
	for i := range res.Databases {

		if !params.GetCommandDef().Params["tags"].FilterFunc(list, &res.Databases[i], params.Tags) {
			continue
		}

		list = append(list, &res.Databases[i])
	}
