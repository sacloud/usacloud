...
func DatabaseMonitorSystemDiskSize(ctx cli.Context, params *params.MonitorSystemDiskSizeDatabaseParam) error {
	...
	// collect values
	totalValues, err := res.FlattenTotalDisk1SizeValue()
	if err != nil {
		return fmt.Errorf("DatabaseMonitorSystemDiskSize is failed: %s", err)
	}
	usedValues, err := res.FlattenUsedDisk1SizeValue()
	if err != nil {
		return fmt.Errorf("DatabaseMonitorSystemDiskSize is failed: %s", err)
	}
