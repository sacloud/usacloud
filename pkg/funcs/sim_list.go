...
func SIMList(ctx cli.Context, params *params.ListSIMParam) error {
	...
	// [manual] add includes
	finder.SetInclude("*")
	finder.SetInclude("Status.sim")

	// call Find()
	res, err := finder.Find()
	if err != nil {
		return fmt.Errorf("SIMList is failed: %s", err)
	}

	list := []interface{}{}
	for i := range res.CommonServiceSIMItems {

		if !params.GetCommandDef().Params["tags"].FilterFunc(list, &res.CommonServiceSIMItems[i], params.Tags) {
			continue
		}

		list = append(list, &res.CommonServiceSIMItems[i])
	}
