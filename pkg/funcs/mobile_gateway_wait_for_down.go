...
func MobileGatewayWaitForDown(ctx cli.Context, params *params.WaitForDownMobileGatewayParam) error {

	client := ctx.GetAPIClient()
	api := client.GetMobileGatewayAPI()
	p, e := api.Read(params.Id)
	if e != nil {
		return fmt.Errorf("MobileGatewayWaitForDown is failed: %s", e)
	}
	...
	if err != nil {
		return fmt.Errorf("MobileGatewayWaitForDown is failed: %s", err)
	}
