...
func (p *updateParameter) buildFlags(fs *pflag.FlagSet) {
	...
	fs.BoolVarP(&p.ClearDescription, "clear-description", "", p.ClearDescription, "Clear the description")
	fs.StringVarP(p.Settings, "settings", "", "", "Process configuration settings JSON or file path; see command help for fields and examples")
...
func (p *updateParameter) buildFlagsUsage(cmd *cobra.Command) {
	...
	}
	{
		...
		fs.SortFlags = false
		fs.AddFlag(cmd.LocalFlags().Lookup("clear-description"))
		fs.AddFlag(cmd.LocalFlags().Lookup("settings"))
		sets = append(sets, &core.FlagSet{
