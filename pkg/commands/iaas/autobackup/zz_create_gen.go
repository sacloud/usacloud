...
func (p *createParameter) buildFlags(fs *pflag.FlagSet) {
	...
	fs.VarP(core.NewIDFlag(&p.DiskID, &p.DiskID), "disk-id", "", "(*required) ")
	fs.StringSliceVarP(&p.Weekdays, "weekdays", "", p.Weekdays, "(*required) options: [all/sun/mon/tue/wed/thu/fri/sat]")
	fs.IntVarP(&p.MaxNumOfArchives, "max-backup-num", "", p.MaxNumOfArchives, "(*required) ")
...
func (p *createParameter) setCompletionFunc(cmd *cobra.Command) {
	cmd.RegisterFlagCompletionFunc("weekdays", util.FlagCompletionFunc("all", "sun", "mon", "tue", "wed", "thu", "fri", "sat"))
