...
func (p *updateParameter) buildFlags(fs *pflag.FlagSet) {
	...
	if p.RecordsData == nil {
		p.RecordsData = pointer.NewString("")
	}
	...
	fs.StringVarP(p.RecordsData, "records", "", "", "")
...
func (p *updateParameter) buildFlagsUsage(cmd *cobra.Command) {
	...
	}
	{
		...
		fs.SortFlags = false
		fs.AddFlag(cmd.LocalFlags().Lookup("records"))
		sets = append(sets, &core.FlagSet{
			Title: "Dns-specific options",
			Flags: fs,
		})
	}
	{
